...
export function deriveShuvbotBranch(input: {
  mode: string;
  runId: string;
  requestedBy: string;
  task: string;
  ...
  return `shuvbot/${slug.slice(0, 48)}-${digest}`;
}

export function assertShuvbotBranchName(branch: string): void {
  if (
    !branch.startsWith("shuvbot/") ||
    branch.length <= "shuvbot/".length ||
    branch.includes("..")
  ) {
...
export async function createOrFastForwardShuvbotBranch(input: {
  cwd: string;
  branch: string;
  startPoint: string;
