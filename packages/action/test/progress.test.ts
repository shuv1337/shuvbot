...
describe("progress comments", () => {
  test("create, debounce, complete, and retain failure state", () => {
    ...
    expect(body).toContain("<!-- reviewbot:progress -->");
    expect(shouldUpdateProgress({ nowMs: 0, debounceMs: 10_000 })).toBe(true);
    expect(shouldUpdateProgress({ previous: { body, updatedAtMs: 0 }, nowMs: 1000, debounceMs: 10_000 })).toBe(false);
    expect(shouldUpdateProgress({ previous: { body, updatedAtMs: 0 }, nowMs: 1000, debounceMs: 10_000, force: true })).toBe(true);

    const failed = completeProgress({ previous: { id: 1, body, updatedAtMs: 0 }, body: "failed", nowMs: 10, failed: true });
