...
export function runnableReviewSkills(input: {
  ...
  config: Pick<ShuvbotConfig, "paths" | "failOn" | "reportOn" | "minConfidence">;
  skills?: readonly ReviewSkill[];
}): ReviewSkill[] {
  const changedPaths = input.files
...
export function getReviewSkill(
  id: string,
  skills: readonly ReviewSkill[] = builtInReviewSkills
