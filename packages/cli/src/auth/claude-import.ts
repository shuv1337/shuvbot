...
export async function runClaudeImport(options: ClaudeImportOptions = {}): Promise<string> {
  ...
  if (options.repo) {
    storeGitHubSecret(
      token,
      options.repo,
      options.secret ?? "CLAUDE_CODE_OAUTH_TOKEN",
      options.spawnSyncImpl ?? spawnSyncLike
    );
  }
...
function storeGitHubSecret(
  ...
  if (result.status !== 0)
    throw new AuthError(`gh secret set failed: ${result.stderr || result.stdout}`);
}
