...
- [x] M8 GitHub integration: the Action routes review mode through the coordinator engine behind an explicit `engine: coordinator` input, with GitHub-native finding lifecycle state in a hidden pull request comment, thread/reply ingestion feeding reconciliation, deterministic posting under runtime policy, non-interactive runtime authentication, artifacts under `$RUNNER_TEMP/shuvbot`, engine/tier/coverage/degraded outputs and workflow summary, and job cancellation forwarded to the runtime. The exit criterion - an opt-in GitHub workflow running the coordinator end to end against a real pull request - has **not** been recorded yet; everything above is covered by integration tests against a scripted engine and a stand-in GitHub API.
...
- `packages/action/src/main.ts` routes to the coordinator when a workflow passes `engine: coordinator`. It never adopts the `review.engine` default on its own: an existing workflow has no pinned runtime installed and no `review.shuvcode.auth = "environment"`, so a silent flip would break it on the next run. Flipping the Action default is deliberately deferred until a real opt-in workflow has run end to end.
